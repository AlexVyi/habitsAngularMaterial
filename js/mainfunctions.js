...
app.controller("mainController",["$compile","$element","$scope", "$interval", "$window", "$firebaseObject","$firebaseArray","$firebaseAuth","$log" ,function($compile, $element,$scope,  $interval, $window, $firebaseObject,$firebaseArray, $firebaseAuth,$log) {
    ...
    $scope.GetGoals=function(){
        ...
        firebaseObj.$bindTo($scope, "dailys").then(function() {
            for (var i = 0; i < Object.keys($scope.dailys).length-2; i++) {
                ...
                $interval($scope.UpdateTime, 1000, 0, true, $scope.dailys["Daily"+i]);
                /*from the doc:
                 $interval(fn, delay, [count], [invokeApply], [Pass]);
                 fn => the function
                 delay=> wait between calls millis
                 count => how often to repeat. 0 (default) means endless loop
                 invokeApply => should apply be invoked
                 Pass=> aditional parameters to be given tu the function


                 old code was $interval($scope.UpdateTime($scope.dailys["Daily"+i])), which is wrong because
                 $scope.UpdateTime($scope.dailys["Daily"+i])) is not a function
                 $scope.UpdateTime is a function
                 and
                 $scope.dailys["Daily"+i] is its parameter

                 This change was made to all $intervals in this function
                 */
            }
        });
        ...
        firebaseObj.$bindTo($scope, "weeklys").then(function() {
            for (var i = 0; i < Object.keys($scope.weeklys).length-2; i++) {
                ...
                $interval($scope.UpdateTime, 1000, 0, true, $scope.weeklys["Weekly"+i]);
            }
        });
        ...
        firebaseObj.$bindTo($scope, "monthlys").then(function() {
            for (var i = 0; i < Object.keys($scope.monthlys).length-2; i++) {
                ...
                $interval($scope.UpdateTime, 1000, 0, true, $scope.monthlys["Monthly"+i]);
            }
        });
    ...
    $scope.SaveChangesGoal=function(editGoal){
        ...
        if(editGoal.goalType.toString()==="Dont"){
            ...
            if(!editGoal.repeats || editGoal.repeats<=0){
